...
int *CALM::CustomMult(int aMultBinMin, int aMultBinMax, int &Nsum)
{
   ...
   if(eventConfig->importMethod==1){
   	pionsFile = new TFile(eventConfig->pionsMultDistrPath.c_str(), "READ");
   	kaonsFile = new TFile(eventConfig->kaonsMultDistrPath.c_str(), "READ");
   	nucleonsFile = new TFile(eventConfig->nucleonsMultDistrPath.c_str(), "READ");
   	lambdasFile = new TFile(eventConfig->lambdasMultDistrPath.c_str(), "READ");
   
	   if (!pionsFile || pionsFile->IsZombie())
	   {
	   	cerr << "Failed to open pionsFile" <<endl;
	   }
	   
	   if (!kaonsFile || kaonsFile->IsZombie())
	   {
	   	cerr << "Failed to open kaonsFile" <<endl;
	   }
	   
	   if (!nucleonsFile || nucleonsFile->IsZombie())
	   {
	   	cerr << "Failed to open nucleonsFile" <<endl;
	   }
	   
	   if (!lambdasFile || lambdasFile->IsZombie())
	   {
	   	cerr << "Failed to open lambdasFile" <<endl;
	   }
	   
	   TH1F *pionsHist = (TH1F*)pionsFile->Get("Pions distribution");
	   TH1F *kaonsHist = (TH1F*)kaonsFile->Get("Kaons distribution");
	   TH1F *nucleonsHist = (TH1F*)nucleonsFile->Get("Nucleons distribution");
	   TH1F *lambdasHist = (TH1F*)lambdasFile->Get("Lambdas distribution");
...
void CALM::SetTotalEnergy(int Nsum, double aEnergy)
{
   ...
   if(eventConfig->importMethod==1){
	   ...
	   TH1F *energyHist = (TH1F*)singleEnergyFile->Get("Energy distribution");
